...
class CourseEngagementVideoPresenter(CourseAPIPresenterMixin, BasePresenter):
    ...
    def _calculate_total_video_segments(self, segment_length, video_duration=None):
        ...
        total_segments = None
        if video_duration:
            total_segments = int(math.ceil(video_duration/segment_length))
        return total_segments

    def _fill_video_timeline_gaps(self, api_response, segment_total=None):
        # fill gaps at the beginning
        first_segment = api_response[0]
        if first_segment['segment'] > 0:
            for i in range(first_segment['segment']):
                api_response.insert(i, self._get_default_video_timeline_segment(i))
